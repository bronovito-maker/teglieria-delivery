  ...
  return <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center backdrop-blur-sm" onClick={onClose}>
    <div className="bg-warm-light w-full sm:max-w-2xl sm:rounded-[2.5rem] rounded-t-[2.5rem] max-h-[92vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
      <div className="p-6 sm:p-8">
        <div className="flex justify-between items-start mb-6"><div><span className="ds-micro-label text-terracotta/60">Configuratore</span><h2 className="text-4xl font-display tracking-tight text-charcoal leading-none mt-2">Crea la tua pizza</h2><p className="text-sm text-charcoal/55 mt-2">Ogni gusto è un contenitore indipendente. Gli ingredienti nello stesso gusto non creano gusti aggiuntivi.</p></div><button onClick={onClose} className="w-10 h-10 rounded-full bg-charcoal/5 text-2xl text-charcoal/50">×</button></div>
        <div className="grid grid-cols-2 gap-2 mb-4">{(["INTERA", "MEZZA"] as PizzaFormat[]).map((value) => <button key={value} onClick={() => setFormatAndGusti(value, value === "INTERA" ? 1 : 1)} className={`rounded-2xl border p-3 text-left ${format === value ? "border-terracotta bg-terracotta/10" : "border-charcoal/10 bg-white"}`}><b className="block text-sm">{PIZZA_BUILDER_CONFIG.formats[value].label}</b><span className="text-xs text-charcoal/50">{PIZZA_BUILDER_CONFIG.formats[value].gusti.length} configurazioni</span></button>)}</div>
